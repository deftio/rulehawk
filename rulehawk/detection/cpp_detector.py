...
class CppDetector(LanguageDetector):
    ...
    def _detect_build_system(self) -> Dict[str, Any]:
        ...
        # Bazel
        elif (self.project_root / "WORKSPACE").exists() or (
            self.project_root / "WORKSPACE.bazel"
        ).exists():
            build["system"] = "bazel"
        ...
        # Autotools
        elif (self.project_root / "configure.ac").exists() or (
            self.project_root / "configure.in"
        ).exists():
    ...
    def _detect_compiler(self) -> Dict[str, Any]:
        ...
        for name, _cmd in compilers_to_check.items():
    ...
    def _detect_testing(self) -> Dict[str, Any]:
        ...
        if (self.project_root / "CMakeLists.txt").exists():
            cmake_content = " ".join(
                self.read_file_lines(self.project_root / "CMakeLists.txt", 200)
            )
    ...
    def _detect_project_type(self) -> str:
        ...
        if (
            self.find_files("**/main.c")
            or self.find_files("**/main.cpp")
            or self.find_files("**/main.cc")
        ):
            return "application"

        # Check for headers only
        h_files = len(self.find_files("**/*.h")) + len(self.find_files("**/*.hpp"))
        impl_files = (
            len(self.find_files("**/*.c"))
            + len(self.find_files("**/*.cpp"))
            + len(self.find_files("**/*.cc"))
        )
